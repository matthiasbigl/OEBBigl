...
<div bind:this={containerRef} class="border border-gray-200 dark:border-gray-700 bg-white dark:bg-black/70 shadow-sm">
	...
	<div class="border-b border-gray-300 dark:border-gray-700 px-4 py-3 bg-gray-100/30 dark:bg-gray-900/30">
		...
		<button 
			...
			class="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-3 hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors duration-200 group px-2 py-1 -mx-2 -my-1 w-full text-left"
			aria-expanded={!isCollapsed}
			aria-controls="station-info-content"
		>
			<div class="flex items-center space-x-3">
				<div class="w-2 h-2 bg-green-500 dark:bg-green-400 pulse-element"></div>
				<div class="text-xs text-gray-600 dark:text-gray-400 font-mono tracking-wider">OPTIONS</div>
				<div class="h-4 w-px bg-gray-400 dark:bg-gray-600"></div>
			</div>
			<div class="flex items-center justify-between w-full md:w-auto">
				<h2 class="text-lg font-bold text-gray-900 dark:text-white truncate font-title">{station.name}</h2>
				<div class="flex items-center space-x-2 md:ml-3">
					<div class="text-gray-600 dark:text-gray-400 transition-transform duration-200 {isCollapsed ? '' : 'rotate-180'}">
						▼
					</div>
				</div>
			</div>
			...
			<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
				<div class="flex items-center space-x-3">
					...
					<button 
						...
						class="flex items-center space-x-2 px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-black/70 hover:bg-gray-100 dark:hover:bg-gray-900 disabled:opacity-60 transition-all duration-200 floating-element"
						title="Manually refresh departure data"
					>
						{#if isRefreshing}
							<LoadingSpinner size="sm" color="white" />
							<span class="text-gray-700 dark:text-gray-300 font-mono">SYNC</span>
						{:else}
							<span class="text-gray-600 dark:text-gray-400">⟳</span>
							<span class="text-gray-700 dark:text-gray-300 font-mono">REFRESH</span>
						{/if}
					</button>
					...
					{#if $hasActiveFilters}
						<button 
							...
							class="flex items-center space-x-2 px-3 py-2 border border-red-500/50 dark:border-red-600/50 bg-red-100/20 dark:bg-red-900/20 hover:bg-red-100/40 dark:hover:bg-red-900/40 transition-all duration-200"
							title="Clear all active filters ({$totalActiveFilters})"
						>
							<span class="text-red-600 dark:text-red-400">✕</span>
							<span class="text-red-700 dark:text-red-300 font-mono text-sm">CLEAR.ALL</span>
						</button>
					{/if}
				</div>
				
				<!-- Status Display -->
				<div class="flex items-center space-x-3 text-gray-600 dark:text-gray-400 text-xs">
					<div class="flex items-center space-x-1">
						<div class="w-1 h-1 bg-green-500 dark:bg-green-400 pulse-element"></div>
						<span class="font-mono">AUTO-SYNC</span>
					</div>
					<div class="text-gray-500 dark:text-gray-500 font-mono">
						{formattedLastUpdate}
					</div>
				</div>
			</div>
